    ...
    public class SeamothFragment3 : DecorationItem
    {
        ...
        public override GameObject GetGameObject()
        {
            ...
            foreach (Transform tr in prefab.transform)
                Logger.Log("DEBUG: 3 Transform name=[" + (!string.IsNullOrEmpty(tr.name) ? tr.name : "?") + "].");
            foreach (Component comp in prefab.GetComponents<Component>())
                Logger.Log("DEBUG: 3 Comp name=[" + (!string.IsNullOrEmpty(comp.name) ? comp.name : "?") + "] type=[" + comp.GetType().ToString() + "].");
#endif
