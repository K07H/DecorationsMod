...
{
    public class WarperPart7 : DecorationItem
    {
        [SetsRequiredMembers]
        public WarperPart7() : base("WarperPart7", LanguageHelper.GetFriendlyWord("WarperPartName") + " (3)", LanguageHelper.GetFriendlyWord("WarperPartDescription"), AssetsHelper.Assets.LoadAsset<Sprite>("warper_icon_7"))
        {
            this.ClassID = "WarperPart7";
            this.PrefabFileName = DecorationItem.DefaultResourcePath + this.ClassID;

            //this.GameObject = AssetsHelper.Assets.LoadAsset<GameObject>("warper_part_7");
            this.GameObject = new GameObject(this.ClassID);

            this.TechType = this.Info.TechType;

            CrafterLogicFixer.WarperPart7 = this.TechType;
        ...
        private static GameObject _warperPart7 = null;

        public override void RegisterItem()
        {
            if (this.IsRegistered == false)
            {
                if (_warperPart7 == null)
                    _warperPart7 = AssetsHelper.Assets.LoadAsset<GameObject>("warper_part_7");

                GameObject model = _warperPart7.FindChild("Model");

                // Translate model
                model.transform.localPosition = new Vector3(model.transform.localPosition.x, model.transform.localPosition.y + 0.045f, model.transform.localPosition.z);

                // Set tech tag
                var techTag = _warperPart7.AddComponent<TechTag>();
                techTag.type = this.TechType;

                // Add prefab identifier
                _warperPart7.AddComponent<PrefabIdentifier>().ClassId = this.ClassID;

                // Add collider
                var collider = _warperPart7.AddComponent<BoxCollider>();
                ...
                var renderers = _warperPart7.GetComponentsInChildren<Renderer>();
                ...
                PrefabsHelper.SetDefaultLargeWorldEntity(_warperPart7);

                // Add sky applier
                PrefabsHelper.UpdateOrAddSkyApplier(_warperPart7, null, renderers);

                // We can pick this item
                PrefabsHelper.SetDefaultPickupable(_warperPart7);

                // We can place this item
                _warperPart7.AddComponent<CustomPlaceToolController>();
                var placeTool = _warperPart7.AddComponent<GenericPlaceTool>();
                ...
                placeTool.ghostModelPrefab = _warperPart7;
                placeTool.mainCollider = collider;
                placeTool.pickupable = _warperPart7.GetComponent<Pickupable>();
                ...
                Nautilus.Handlers.SpriteHandler.RegisterSprite(this.TechType, AssetsHelper.Assets.LoadAsset<Sprite>("warper_icon_7"));
        ...
        {
            GameObject prefab = GameObject.Instantiate(_warperPart7);
            ...
            fabricatingA.posOffset = new Vector3(0.01f, 0f, 0.04f);
