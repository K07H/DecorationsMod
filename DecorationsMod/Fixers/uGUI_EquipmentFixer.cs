        ...
        public static bool CanSwitchOrSwap_Prefix(uGUI_Equipment __instance, ref ItemAction __result, string slotB)
        {
            ...
            if (ModdedBatteriesFixer.BatteriesTechTypes().Contains(techType))
            {
                if (Equipment.GetSlotType(slotB) == EquipmentType.BatteryCharger)
                {
                    ...
                    {
#if DEBUG_PLACE_TOOL
                        Logger.Debug("DEBUG: CanSwitchOrSwap returns SWITCH battery for " + techType.AsString(false));
#endif
                    ...
                    {
#if DEBUG_PLACE_TOOL
                        Logger.Debug("DEBUG: CanSwitchOrSwap returns SWAP battery for " + techType.AsString(false));
#endif
                        __result = ItemAction.Swap;
                        return false;
                    }
#if DEBUG_PLACE_TOOL
                    Logger.Debug("DEBUG: CanSwitchOrSwap returns NONE battery for " + techType.AsString(false));
#endif
            ...
            else if (ModdedBatteriesFixer.PowercellsTechTypes().Contains(techType))
            {
                if (Equipment.GetSlotType(slotB) == EquipmentType.PowerCellCharger)
                {
                    ...
                    {
#if DEBUG_PLACE_TOOL
                        Logger.Debug("DEBUG: CanSwitchOrSwap returns SWITCH powercell for " + techType.AsString(false));
#endif
                        __result = ItemAction.Switch;
                        return false;
                    }
                    ...
                    {
#if DEBUG_PLACE_TOOL
                        Logger.Debug("DEBUG: CanSwitchOrSwap returns SWAP powercell for " + techType.AsString(false));
#endif
                        __result = ItemAction.Swap;
                        return false;
                    }
#if DEBUG_PLACE_TOOL
                    Logger.Debug("DEBUG: CanSwitchOrSwap returns NONE powercell for " + techType.AsString(false));
#endif
                    __result = ItemAction.None;
                    return false;
                }
...
#if DEBUG_PLACE_TOOL
            Logger.Debug("DEBUG: CanSwitchOrSwap returns NONE for " + techType.AsString(false) + " and origin function will get called.");
#endif
            __result = ItemAction.None;
            return true;
        }
