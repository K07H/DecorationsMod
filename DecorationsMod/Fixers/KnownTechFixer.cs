    ...
    public class KnownTechFixer
    {
        ...
        public static bool AddNotification(TechType techType, bool builderNotification = true, bool blueprintsNotification = true, bool craftNotification = false)
        {
            ...
            if (KnownTechFixer.AddedNotifications.ContainsKey(techTypeId) && !KnownTechFixer.AddedNotifications[techTypeId])
            {
                if (builderNotification)
                {
                    ...
                    catch { Logger.Warning("WARNING: Could not add builder notification for tech type [" + techType.AsString() + "]."); }
                }
                if (blueprintsNotification)
                {
                    ...
                    catch { Logger.Warning("WARNING: Could not add blueprint notification for tech type [" + techType.AsString() + "]."); }
                }
                if (craftNotification)
                {
                    ...
                    catch { Logger.Warning("WARNING: Could not add craft notification for tech type [" + techType.AsString() + "]."); }
                }
        ...
        public static void LoadAddedNotifications(string saveGame)
        {
            ...
            if (saveDir != null)
            {
                if (Directory.Exists(saveDir))
                {
                    ...
                    {
                        Logger.Info("INFO: Loading discoveries from \"" + saveFile + "\".");
                        int cnt = 0;
                        ...
                        Logger.Info("INFO: Discoveries loaded. Player made {0}/{1} discoveries ({2} remaining).", cnt, AddedNotifications.Count, AddedNotifications.Count - cnt);
                    }
                    else
                        Logger.Info("INFO: No discoveries saved at \"" + saveFile + "\".");
                }
                else
                    Logger.Info("INFO: No save directory found for discoveries at \"" + saveDir + "\".");
            }
            else
                Logger.Info("INFO: Could not find save slot for discoveries.");
        }
        ...
        public static void SaveAddedNotifications()
        {
            ...
            if (!string.IsNullOrEmpty(toSave))
            {
                ...
                Logger.Info("INFO: Saving {0}/{1} discoveries to \"{2}\" (Discovered TechTypes: {3}).", cnt, AddedNotifications.Count, saveFile, toLog);
