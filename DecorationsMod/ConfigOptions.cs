...
namespace DecorationsMod
{
    class ConfigOptions : ModOptions
    {
                ...
                if (ConfigSwitcher.OpenConfiguratorLastState != ConfigSwitcher.OpenDecorationsModConfigurator)
                {
                    ...
                    if (File.Exists(configuratorPath))
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            // Log error
                            Logger.Error("ERROR: Unable to open configurator. Exception=[" + ex.ToString() + "]");
        ...
        private static string ConfiguratorPath() => FilesHelper.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "Configurator/", "DecorationsModConfigurator.exe");
