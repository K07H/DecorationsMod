    ...
    public class CustomPictureFrameController : MonoBehaviour, IProtoEventListener
    {
        ...
        private void InitRandomMode(string folderPath)
        {
            ...
            if (this._pf.fileName != null)
                ...
                Logger.Info("INFO: Could not find a valid image inside custom picture frame images folder at \"" + folderPath + "\".");
        }
        ...
        private void InitSlideshowMode(string folderPath)
        {
            ...
            if (files != null)
            {
                ...
                if (this.SlideshowImages.Count > 0)
                {
                    ...
                    Logger.Info("INFO: Could not find any image inside custom picture frame images folder at \"" + folderPath + "\".");
            }
            else
                Logger.Info("INFO: Could not find any files inside custom picture frame images folder at \"" + folderPath + "\".");
        ...
        private void InitImagesFolder(PrefabIdentifier id, PictureFrame pf, bool initRandom = false, bool initSlideshow = false)
        {
            ...
            if (!Directory.Exists(rootFolderPath))
            {
                ...
                catch { Logger.Warning("WARNING: Could not create directory for custom picture frames at \"" + rootFolderPath + "\"."); }
            }
            if (Directory.Exists(rootFolderPath))
            {
                ...
                if (!Directory.Exists(folderPath))
                {
                    ...
                    catch { Logger.Warning("WARNING: Could not create images directory for custom picture frame at \"" + folderPath + "\"."); }
                    if (Directory.Exists(folderPath))
                    {
                        try
                        {
                        ...
                        catch { Logger.Warning("WARNING: Could not add config file into custom picture frame images folder."); }
                        try { Process.Start(folderPath); }
                        catch { Logger.Warning("WARNING: Could not open custom picture frame images folder at \"" + folderPath + "\"."); }
                    }
                }
                if (Directory.Exists(folderPath))
                {
                            ...
                            foreach (string line in lines)
                                if (line.StartsWith("Delay="))
                                {
                                    if (line.Length > 6 && int.TryParse(line.Substring(6), NumberStyles.Integer, CultureInfo.InvariantCulture, out int delay) && delay > 0)
                                        ...
                                        Logger.Warning("WARNING: The line \"" + line + "\" does not have a correct value (must be an integer greater than 0). Default value will be used.");
                                    break;
                                }
                    }
                    ...
                    Logger.Warning("WARNING: Could not find custom picture frame images folder at \"" + folderPath + "\".");
            }
        }
        ...
        public void OnProtoDeserialize(ProtobufSerializer serializer)
        {
            ...
            if (File.Exists(filePath))
            {
                ...
                if (sizes != null && sizes.Length >= 10 && sizes.Length <= 14)
                {
                    ...
                    Logger.Debug("DEBUG: Current image in picture frame: fileName=[" + (string.IsNullOrEmpty(pf.fileName) ? "?" : pf.fileName) + "]");
#endif
                }
            }
        }
